...
public class CityAutoCompleteServiceImpl implements AutoCompleteService<City> {
    ...
    public void initCache() {
		...
		String maxItems;
		timeToLive = context.getEnvironment().getProperty("cache.city.timeToLive");
		if (timeToLive != null) {
			timerInterval = context.getEnvironment().getProperty("cache.city.timerInterval");
			maxItems = context.getEnvironment().getProperty("cache.city.maxItems");
		} else {
			LOGGER.info(
					"city specific cache settings not found in application.properties. Applying default cache settings");
			timeToLive = context.getEnvironment().getProperty("cache.default.timeToLive");
			timerInterval = context.getEnvironment().getProperty("cache.default.timerInterval");
			maxItems = context.getEnvironment().getProperty("cache.default.maxItems");
		}
    ...
    public List<City> search(final String key) {
		...
		result = cities.stream().map(mstCity -> {
			City city = new City();
